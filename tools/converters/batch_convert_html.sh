...
# 检查当前目录是否为项目根目录
if [ ! -d "tools" ] || [ ! -d "expanded_schemas" ]; then
    print_error "请在项目根目录执行此脚本"
    print_error "当前目录: $PWD"
    exit 1
fi

# 切换到 tools/converters 目录
print_info "切换到 tools/converters 目录..."
cd tools/converters

# 检查必要的文件和目录
if [ ! -f "./yaml_to_html.py" ]; then
    print_error "转换脚本不存在: ./yaml_to_html.py"
    exit 1
fi
...
for yaml_file in ../../expanded_schemas/*.expanded.yaml; do
    ...
    # 执行转换
    if "$PYTHON_BIN" ./yaml_to_html.py "$yaml_file" -o "$html_file" -l "$LANGUAGE_MODE"; then
...
# 生成索引文件
print_info "生成索引文件..."
"$PYTHON_BIN" ./generate_index.py -l "$LANGUAGE_MODE" -d "$OUTPUT_DIR"
